...
def test_step_basic_update(simple_params_and_grads):
    ...
    optimizer = RMSProp(learning_rate=0.001, rho=0.9, eps=1e-8, momentum=0.0)
    
    updated_params = optimizer.update(params, grads)
    
    assert optimizer.eg2 is not None
    assert optimizer.velocity is None 
    assert len(optimizer.eg2) == len(params)
...
def test_step_with_momentum(simple_params_and_grads):
    params, grads = simple_params_and_grads
    optimizer = RMSProp(learning_rate=0.001, rho=0.9, eps=1e-8, momentum=0.9)
...
def test_gradient_clipping(simple_params_and_grads):
    params, grads = simple_params_and_grads
    optimizer = RMSProp(learning_rate=0.001, clip_norm=0.5)
...
def test_learning_rate_decay(simple_params_and_grads):
    params, grads = simple_params_and_grads
    optimizer = RMSProp(learning_rate=0.001, decay_rate=0.5)
...
def test_regularization(simple_params_and_grads):
    params, grads = simple_params_and_grads
    optimizer = RMSProp(learning_rate=0.001, reg_type='l2', weight_decay=0.1)
...
def test_history_tracking(simple_params_and_grads):
    params, grads = simple_params_and_grads
    optimizer = RMSProp(track_history=True, track_interval=2)
...
def test_on_step_callback(simple_params_and_grads):
    ...
    optimizer = RMSProp(on_step=on_step)
    updated_params = optimizer.update(params, grads)
...
def test_repr():
    ...
    assert 'RMSProp' in repr_str
    assert 'lr=0.001' in repr_str
    assert 'rho=0.9' in repr_str
